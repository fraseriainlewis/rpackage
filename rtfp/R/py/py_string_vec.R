...
if(1){
  ...
  chain5<-rep(NA,n_samples);
  chain6<-rep(NA,n_samples);
  ...
  for(i in 0:(n_samples-1)){ # 0-indexing
    ...
    chain5[i+1]<-par_samples[[i]]$numpy()[5]
    chain6[i+1]<-par_samples[[i]]$numpy()[6]
  }
